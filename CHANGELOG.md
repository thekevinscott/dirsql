...
- **Breaking: the `[dirsql].root` config key is gone (#540, epic #528).** A config file describes content (tables, hooks), not where you are — the index root is an operational fact owned by the runner. With the strict parser (#536), an old config carrying `root` now fails loudly with serde's `unknown field 'root'` rather than being silently honored. See `MIGRATIONS.md`.

### Changed

- **Breaking: the index root is now the runner's, never the config file's (#540, epic #528).** One uniform rule across every surface: the index root is the explicit `.root(...)` (SDK) / invocation cwd (CLI) — the config file's parent directory no longer plays any part in root derivation. The default flow is unchanged (`dirsql` with `./.dirsql.toml`: parent = `.` = cwd). Behavior changes when the config lives elsewhere: `dirsql --config /elsewhere/.dirsql.toml` (and the SDK's `from_config_path("/elsewhere/…")` / `config=` without an explicit root) previously rooted at `/elsewhere`, and now root at the invocation cwd. The Rust builder's explicit-root-vs-config-root collision warning is deleted (nothing to collide with), and a builder with neither `.root()` nor `.config()` no longer errors — it roots at the cwd. See `MIGRATIONS.md`.
- **Breaking: unknown `.dirsql.toml` keys are now a hard error (#536, epic #528).** The TOML parser previously dropped unknown keys silently at every level, so a typo (`glbo`, `persistpath`) or a removed key no-opped instead of failing. Every raw config struct now sets `deny_unknown_fields` (top level, `[dirsql]`, `[[table]]`, `[[dirsql.extension]]`), so an unknown key is a parse error naming the offending key — surfaced like every other config error: the server degrades (503 with the diagnostic) and `dirsql query` exits non-zero. See `MIGRATIONS.md`.
- **Breaking: `on-file` no longer appends `{path}` when the template omits it (#538, epic #528).** Token interpolation is now the only channel for passing data to an `on-file` command — the append-if-absent ergonomic is gone. Previously `on-file = "cat"` implicitly received the matched file's relative path as a trailing argument, behaving like `on-file = "cat {path}"`; now the path only reaches the command where the template references `{path}` explicitly. A template that omits `{path}` runs with no path (e.g. `cat` reads its null stdin, produces no output), so the file is skipped per-file with a stderr warning and contributes no rows. The `Placeholder::append` constructor and the `append_if_absent` field on the shared command runner are removed; `Placeholder::new` (substitute-only) remains. See `MIGRATIONS.md`.
- **Breaking: the `on-file` `{path}` token now interpolates the file's absolute path (#542, part of #528).** `{path}` was the matched file's path relative to the index root; it is now the **absolute** path. An absolute path is self-sufficient from any working directory, so an `on-file` command still resolves it when its config file lives outside the index (the case #528's repeatable `--config` enables) — a root-relative `{path}` would not. `{root}` is unchanged, and a command wanting the relative form derives it from the two (`relpath({path}, {root})`). This changes only the hook token; the `path` **column** (stat virtuals) stays root-relative — that's row data, not command plumbing. Commands that concatenate `{path}` onto another path (`{root}/{path}`) or store it as a column value now see absolute paths. See `MIGRATIONS.md`.
- **Breaking (Rust SDK): `DirSQLBuilder::persist(bool)` + `.persist_path(path)` collapsed into one optional-path method (#551).** The builder now exposes a single `.persist(path: Option<impl AsRef<Path>>)`: `None` enables persistence at the default `<root>/.dirsql/cache.db`, `Some(path)` enables it at `path`. The separate `.persist_path()` setter is removed, and persistence is off unless `.persist(..)` is called (the old `.persist(false)` no-op has no replacement — simply don't call it). Cache/reconcile behavior is unchanged; this only affects the Rust builder call site. The Python and TypeScript constructors keep their `persist` / `persist_path` (`persistPath`) parameters. See `MIGRATIONS.md`.
- **Python wheels are now stable-ABI (abi3) — one `cp311-abi3` wheel per platform instead of one per interpreter (#487, epic #480).** The pyo3 binding enables `abi3-py311`, so maturin emits a single `cp311-abi3` wheel per target that loads on every CPython ≥ 3.11 (matching `requires-python = ">=3.11"`), rather than a separate `cp311`/`cp312`/`cp313`/`cp314` wheel each. `pip install dirsql` and `uvx dirsql` behave identically across 3.11–3.14; the only observable difference is the wheel filename tag (`…-cp311-abi3-<platform>.whl`). This collapses the release/precheck build matrix' per-Python-version wheel axis 4×. No `putitoutthere.toml` change: upstream's abi3 detector reads the feature straight off the binding crate's `pyo3` dependency and collapses the fan automatically. See `MIGRATIONS.md`.
...
- **`.dirsql.toml`: `on-file` per-table command event — the first
  ...
  *contents*: `dirsql` runs the command once per file (the command reads the
  ...
  and `{root}` (the index root), substituted where the template references them.
  The command runs in the config file's
...
- **Rust core: a reusable command runner (`dirsql::command::run_command`), the
  ...
  untrusted values stay a single injection-safe argument. Runs the child in a given working directory
