...
| `persist` | boolean | `false` | Keep the SQLite index at a named on-disk path between runs. When `false`, the index is an anonymous SQLite temp database, rebuilt from your files on every startup and deleted when the process exits; its pages spill to the system temp directory rather than RAM (set `SQLITE_TMPDIR` to steer placement, e.g. off a tmpfs mount). |
