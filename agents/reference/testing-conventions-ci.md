...
- **A missing e2e receipt is OURS to write, never a tool bug.** `e2e-verify` runs unconditionally but decides on the `base...HEAD` diff: `--scope <path>` narrows what counts as scoped source, so test/docs-only commits owe nothing, and a branch that did change the source reds until its diff carries a receipt under `<package>/e2e-attestations/`. The fix is always `just e2e-attest-<pkg>` on that branch. **Squash merges and rebases no longer strand a receipt** -- the gate compares diffs, not commit SHAs, so the receipt's `commit` field pointing at a dangling pre-squash commit is not a red (the single-file `e2e-attestation.json` layout this replaced did red that way).
