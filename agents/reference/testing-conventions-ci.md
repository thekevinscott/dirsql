...
#### Path triggers: deciding which workflows a change should run (#834)

The spec table in #834 enumerates the cases; this is the rule that generates it:

> **A workflow runs iff the change could alter its verdict.**

One rule, applied uniformly, no per-workflow judgment. Worked consequences:

- **A workflow file runs its own workflow.** `ts-lint.yml` *defines* TS Lint's verdict, so editing it must run TS Lint. This holds regardless of what the run costs -- editing `release-precheck.yml` re-runs the full 13-cell build matrix (17 checks). Cost is not an input to the rule; a cost-based carve-out is a judgment call at every new workflow and cannot be mechanically enforced by `dirsql-checks ci-paths`.
- **Docs and agent instructions run nothing.** `changelog-gate`'s verdict is a function of `packages/**` paths plus commit-body `skip-changelog:` trailers, and nothing else. On an `AGENTS.md`-only PR `changed_packages()` returns empty and the gate takes its `"No package source changed; nothing to enforce."` early return -- it cannot fail, so running it is pure cost at zero signal. Same for every `Conventions` lane and `Release Precheck`. This is why row 1 of the spec table is "nothing".
- **The rule is what #862 asserts.** `dirsql-checks ci-paths` evaluates workflow `paths:` filters and Mergify `if:` anchors against every tracked file. It can only check a rule stated as a total function of the changed paths, which is the reason the rule admits no exceptions.

- **`actionlint` is configured but never invoked (verified 2026-08-12).** `.github/actionlint.yaml` carries a real per-file ignore for the `cargo-dist`-generated `release.yml`, but nothing runs the linter: no workflow step, no justfile recipe, no git hook, no `internals/checks` subcommand. The only other repo-wide references are error-message strings. So **the self-trigger is currently the only thing standing between a broken workflow file and the next contributor's red PR** -- do not weaken row 16 on the assumption that a YAML linter is backstopping it. Wiring actionlint up would be a genuine addition, not a substitute: it catches syntax and expression errors, not a `run:` line invoking a script that `package.json` no longer defines.
