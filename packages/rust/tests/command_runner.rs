...
//! spawning, stdin, and exit handling — which the Rust `unit lint`
//! isolation rule keeps out of colocated unit tests. They are Unix-only (they
//! shell out to `sh`/`cat`); the Rust CI test job runs on Linux.
#![cfg(unix)]
...
fn runs_in_the_given_cwd_so_relative_paths_resolve() {
    ...
    let out = run_command("cat marker.txt", &[], dir.path(), None).expect("command succeeds");
...
fn inherits_the_parent_environment() {
    ...
    let out = run_command(r#"sh -c 'printf %s "$PATH"'"#, &[], dir.path(), None)
        .expect("command succeeds");
...
fn writes_the_stdin_payload_to_the_child() {
    let dir = TempDir::new().unwrap();
    let out =
        run_command("cat", &[], dir.path(), Some(b"chatter\nPAYLOAD\n")).expect("command succeeds");
    assert_eq!(out.payload, "PAYLOAD");
}
...
fn non_zero_exit_is_an_error_carrying_the_stderr_tail() {
    let dir = TempDir::new().unwrap();
    let err = run_command("sh -c 'echo boom >&2; exit 3'", &[], dir.path(), None)
...
fn a_signal_terminated_child_reports_code_signal() {
    let dir = TempDir::new().unwrap();
    let err =
        run_command("sh -c 'kill -TERM $$'", &[], dir.path(), None).expect_err("command fails");
    match err {
...
#[test]
fn a_timeout_wrapped_command_that_overruns_reports_nonzero_exit() {
    // The runner itself is unbounded; bounding is the command's job via
    // timeout(1), whose kill surfaces as an ordinary non-zero exit.
    let dir = TempDir::new().unwrap();
    let err = run_command("timeout 0.2 sleep 30", &[], dir.path(), None)
        .expect_err("timeout(1) kills the child");
    assert!(
...
fn an_invalid_template_is_rejected_before_spawning() {
    let dir = TempDir::new().unwrap();
    let err = run_command("   ", &[], dir.path(), None).expect_err("invalid");
    assert!(
...
fn a_large_stdin_payload_flows_without_deadlocking() {
    ...
    let out = run_command("cat", &[], dir.path(), Some(&payload)).expect("command succeeds");
