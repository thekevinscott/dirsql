...
//
// The core loads config extensions literally — it has no `require.resolve`,
// so it cannot resolve a bare **package name**. When any TOML config in argv
// names an extension by package name, the shared SDK resolver resolves every
// config's entries and this launcher passes the resolved literal paths on via
// repeatable `--extension` flags; the core then loads those and ignores the
// configs' own extension entries.
//
