// Packaging smoke test for the published npm artifact set -- the `smoke`
// tier (build -> pack -> install -> run). Distinct from the e2e tier
// (CLI features) and from packaging.yml, which only checks that no test
// files ship in the artifact; this proves the artifact actually installs
// and runs.
//
...
//
// This is the in-CI publishability gate: red here means the npm release
...
//   what `src/cli/resolve-binary.ts` consumes (`require.resolve(
