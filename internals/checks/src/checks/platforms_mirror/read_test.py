"""Colocated unit test for the platforms-mirror reader (isolation -- a scratch
file, never the repo's real platforms.py / platforms.ts).
"""
...
def describe_read():
    ...
    def it_reads_a_file_whole(tmp_path):
        path = tmp_path / "platforms.ts"
        path.write_text("one\ntwo\n", encoding="utf-8")
