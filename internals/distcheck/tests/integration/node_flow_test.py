...
suite exercised. Skips when the build prerequisites (the staged host CLI binary
...
from distcheck.node_flow.gate import run
...
def test_npm_installed_cli_packs_installs_and_runs():
    ...
    staged = os.path.join(
        _TS, "build", f"bundled-cli-{host.slug}", host.bin_name
    )
    if not os.path.exists(staged):
        pytest.skip("staged CLI binary absent (pnpm build in packages/ts)")
