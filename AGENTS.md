...
`conventions.yml`'s `internals-checks` call (#494/#503) gates the repo-tooling `internals/checks` package the same way: `colocated-test`, `unit-lint`, `unit-coverage`, and `mutation` at `internals/checks/src`. No `integration-lint`/`e2e-verify` yet -- the package has no `tests/integration/` or `tests/e2e/` tier (see the follow-up epic tracking their introduction).
