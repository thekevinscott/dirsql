...
The exemption count is now **zero**. The last entry -- the python barrel-test isolation waiver (`[[python.exempt]] path = "__init___test.py" rules = ["unmocked-collaborator"]`) -- was removed once testing-conventions#382 / PR #384 brought the python `unmocked-collaborator` rule to parity with TS: a bare package-relative import (`from . import <names>`) in a barrel test now resolves to the SUT and passes, so the python barrel is verified with no waiver, exactly like `index.test.ts`. Exemption entries carry a `path` (relative to the scanned source dir), the `rules` waived, and a required `reason`; the CLI **rejects a stale entry whose `path` matches no file**, so it self-cleans. Adding a *new* untested source file fails the gate -- an exemption is never the escape hatch.
