...
The zero-exemptions goal is **temporarily suspended** pending #519/#520: the only exemptions today waive `unknown-tier` for the `tests/binding/` (11 files/SDK) and `tests/smoke/` (1 file/SDK) suites in both the Python and TypeScript packages. testing-conventions#417 (live on `@v0`) derives `integration lint`'s subjects from the package root and recognizes only `tests/integration/` and `tests/e2e/` as suite tiers, so dirsql's two extra tiers trip `unknown-tier` repo-wide. Each waiver's `reason` points at its relocation issue -- binding folds into `tests/integration/binding/` (#519), smoke moves to an internals package (#520) -- and since the CLI **rejects a stale entry whose `path` matches no file**, those moves are forced to delete their own entries. The prior state was zero: the last standing entry -- the python barrel-test isolation waiver (`[[python.exempt]] path = "__init___test.py" rules = ["unmocked-collaborator"]`) -- was removed once testing-conventions#382 / PR #384 brought the python `unmocked-collaborator` rule to parity with TS (a bare package-relative import `from . import <names>` in a barrel test now resolves to the SUT and passes). Exemption entries carry a `path` (relative to the scanned source dir for source-file rules, but the derived package root for the suite-tier `unknown-tier` rule), the `rules` waived, and a required `reason`. Adding a *new* untested source file fails the gate -- an exemption is never the escape hatch.
