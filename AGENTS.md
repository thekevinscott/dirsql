...
Exactly **one** exemption remains, and it is **tool-blocked, not a barrel waiver**: `[[python.exempt]] path = "__init___test.py" rules = ["unmocked-collaborator"]`. Verifying a re-export barrel requires importing the surface it re-exports; the python isolation rule flags that import as an unmocked collaborator (it even flags `__all__`, which is *defined in* `__init__.py`), while the identical TS pattern (`index.test.ts` importing `./index.js`) passes. Mocking the collaborators would make the barrel test assert nothing, so we waive isolation **on the test** (keeping the barrel genuinely verified) rather than waive colocated-test on the barrel (leaving it untested). This is a testing-conventions python/ts parity gap filed upstream; when it lands, this last entry is deleted and the exemption count is zero. Exemption entries carry a `path` (relative to the scanned source dir), the `rules` waived, and a required `reason`; the CLI **rejects a stale entry whose `path` matches no file**, so it self-cleans. Adding a *new* untested source file fails the gate -- an exemption is never the escape hatch.
...
**Shared-core changes stale both bindings -- freshness not yet CI-gated (interim gap).** The shared Rust core (`packages/rust/src`) is compiled into both bindings but lives in neither binding's subtree, and `e2e verify --scope` requires the scope to be a **descendant of the caller's `path`** -- so a binding caller structurally cannot point its freshness walk at the core. testing-conventions has no reusable equivalent for this monorepo-shared-source case, so it is **filed upstream** (testing-conventions `e2e-verify` shared-core scope) rather than kept as bespoke tooling. The former bespoke gate (`e2e_core_freshness.py` in the deleted `e2e-attestation.yml`) was removed as part of whole-hog adoption. **Until the upstream fix lands, a non-`cli` change under `packages/rust/src/**` is NOT freshness-gated against the binding attestations in CI** -- the binding CI tier (`tests/binding`, real core) still runs, but the e2e-freshness promise is not enforced for core-only changes. **Re-attest each binding after a shared-core change by hand** (both `packages/python/e2e-attestation.json` and `packages/ts/e2e-attestation.json`). **`cli`-only** core source (`packages/rust/src/cli/**`, `packages/rust/src/bin/**`) is feature-gated behind the `cli` Cargo feature and never compiled into the bindings, so it never staled the bindings anyway (#328 was exactly this shape).
