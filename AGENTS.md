...
`conventions.yml`'s `internals-checks` call (#494/#503) gates the repo-tooling `internals/checks` package the same way: `colocated-test`, `unit-lint`, `unit-coverage`, `mutation`, and `e2e-verify` at `internals/checks/src`, plus a separate `internals-checks-integration` call running `integration-lint` at `internals/checks/tests/integration` (#509). `tests/integration/` exercises each check's `gate.run()` against real collaborators (real git, real pytest subprocess); `tests/e2e/` spawns the packaged `dirsql-checks` CLI with nothing mocked, gated by `internals/checks/e2e-attestation.json` per the E2E Attestation convention below.
