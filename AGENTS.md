...
- **Smoke tests**: `tests/smoke/` -- *functional* publishability: build, pack, install, and run the published artifact (`build.test.ts`, `build_test.py`). **Run in CI.** Distinct from the **`packaging` gate** (testing-conventions, run via `conventions.yml` for all three languages), which only asserts no test files *ship* in the built artifact and never installs or runs it.
...
`conventions.yml`'s `internals-checks` call (#494/#503) gates the repo-tooling `internals/checks` package the same way: `colocated-test`, `unit-lint`, `unit-coverage`, `mutation`, and `e2e-verify` at `internals/checks/src`, plus a separate `internals-checks-integration` call running `integration-lint` at `internals/checks/tests/integration` (#509) -- mirroring the pre-existing `python-integration` / `typescript-integration` split. `integration-lint` needs its own call today: a live CI run confirmed the current `@v0` release scans exactly the given `path` for `integration lint` rather than deriving `<package root>/tests/integration/` the way `thekevinscott/testing-conventions#417` / `docs/monorepo.md`'s "The test tiers" describes, so pointing it at `internals/checks/src` wrongly flagged the colocated *unit* tests there. Once #417 lands and is confirmed live (`SCAN_PATH` for `integration lint` resolving to `tests/integration` when `path` is the source dir), fold `internals-checks-integration` back into `internals-checks` and delete the separate call. `tests/integration/` exercises each check's `gate.run()` against real collaborators (real git, real pytest subprocess); `tests/e2e/` spawns the packaged `dirsql-checks` CLI with nothing mocked, gated by `internals/checks/e2e-attestation.json` per the E2E Attestation convention below.
...
**Exemptions.** The goal is **zero** exemptions, and barrels are no longer an excuse for one. A re-export barrel gets a **colocated test that asserts its public surface** (TS `src/index.ts` ↔ `index.test.ts`, python `dirsql/__init__.py` ↔ `__init___test.py`), exactly as any module would -- it is *tested*, not exempted, so a broken re-export is caught. An `__init__.py` carrying no executable logic is made **truly empty** (0 bytes), which the tool auto-skips with no config entry. A package shell left dead by a feature removal is **deleted**, not parked behind an exemption. When a "barrel" actually holds logic, the fix remains to **extract that logic into colocated-tested modules** (#239). The npm `bin` shim `src/cli/dirsql.ts` is likewise *not* exempt: its error-handling lives in the unit-tested `cli/run-cli.ts`, leaving a trivial `runCli()` shim covered by a mocked smoke-test.

The zero-exemptions goal is **temporarily suspended** pending #520: the only exemption today waives `unknown-tier` for the `tests/smoke/` (1 file/SDK) suite in both the Python and TypeScript packages. testing-conventions#417 (live on `@v0`) derives `integration lint`'s subjects from the package root and recognizes only `tests/integration/` and `tests/e2e/` as suite tiers, so the smoke tier trips `unknown-tier` repo-wide. The waiver's `reason` points at its relocation issue -- smoke moves to an internals package (#520) -- and since the CLI **rejects a stale entry whose `path` matches no file**, that move is forced to delete the entries. (The binding waivers were already removed by #519, which relocated that suite into the recognized tier `tests/integration/binding/`.) The prior state was zero: the last standing entry -- the python barrel-test isolation waiver (`[[python.exempt]] path = "__init___test.py" rules = ["unmocked-collaborator"]`) -- was removed once testing-conventions#382 / PR #384 brought the python `unmocked-collaborator` rule to parity with TS (a bare package-relative import `from . import <names>` in a barrel test now resolves to the SUT and passes). Exemption entries carry a `path` (relative to the scanned source dir for source-file rules, but the derived package root for the suite-tier `unknown-tier` rule), the `rules` waived, and a required `reason`. Adding a *new* untested source file fails the gate -- an exemption is never the escape hatch.
...
5. **Smoke tests validate the built/packed artifact, not features:** install the published package and run the CLI. A no-mock CI tier (functional publishability); complements the `packaging` gate's file-hygiene check.
