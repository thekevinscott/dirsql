...
**Python mutation is still bespoke** (`.github/workflows/python-mutation.yml`, `maturin develop`): the CLI drives cosmic-ray via the **testing-conventions pip package** (which ships the cosmic-ray adapter the CLI spawns as `python3 -m testing_conventions.mutation.main`), installed into the maturin venv so the `python3 -m pytest` baseline resolves the built `_dirsql`. Moving it into the reusable workflow is tracked in #417's parent (#240), pending the python single-env build fix.
