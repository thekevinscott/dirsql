...
`conventions.yml`'s `internals-checks` call (#494/#503) gates the repo-tooling `internals/checks` package the same way: `colocated-test`, `unit-lint`, `integration-lint`, `unit-coverage`, `mutation`, and `e2e-verify`, all in the one call scoped at `internals/checks/src` (#509). `integration-lint` runs in this same call rather than a separate `path`/job because testing-conventions resolves the integration suite from the *derived package root* (`<package root>/tests/integration/`), independent of `path` -- see `thekevinscott/testing-conventions#417` / `docs/monorepo.md`'s "Everything derives from the package". `tests/integration/` exercises each check's `gate.run()` against real collaborators (real git, real pytest subprocess); `tests/e2e/` spawns the packaged `dirsql-checks` CLI with nothing mocked, gated by `internals/checks/e2e-attestation.json` per the E2E Attestation convention below. (The pre-existing `python-integration` / `typescript-integration` bespoke calls still split `integration-lint` into their own job -- they predate #417 landing and are a candidate for the same consolidation as a follow-up, not part of #509.)
