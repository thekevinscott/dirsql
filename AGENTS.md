...
Six per-domain workflows call the `testing-conventions` reusable workflow at the **moving tag `@v0`** -- `dirsql-python-ci.yml`, `dirsql-typescript-ci.yml`, `dirsql-rust-ci.yml`, `internals-checks-ci.yml`, `internals-distcheck-ci.yml`, `plugin-dirsql-embedding-ci.yml` (#861 split the single `conventions.yml` into these, one per domain, so path filters could triage per lane). The essentials: a removed/renamed input startup-fails the whole *calling* workflow on `main` and every PR (0 jobs, no job logs -- WebFetch the run's `html_url` and read the annotation first, never hypothesize), and since every caller passes the same inputs, one bad input reds all six; an unknown `testing-conventions.toml` key fails EVERY gate; read upstream behavior at a pinned sha and trust its `MIGRATIONS.md` over probing; a missing e2e receipt is ours to write, never a tool bug; never retire a bespoke gate before a green proof. Full operational lore, the per-language gate adoption map, and the two permanent exceptions (Rust unit-coverage stays bespoke; Rust has no e2e-verify): `agents/reference/testing-conventions-ci.md`.
...
The colocation rule is a blocking CI gate ([`testing-conventions`](https://github.com/thekevinscott/testing-conventions), wired in the per-domain CI workflows + `testing-conventions.toml`): every source file needs a colocated unit test (Rust: inline `#[cfg(test)]`), a PR-only co-change check flags modified sources whose test didn't change, and `unit lint` enforces **isolation** -- a unit test must mock every collaborator (Python `patch(...)`, TS `vi.mock`, Rust trait double or relocate the effectful test), never weaken the test. The gates also cover the two binding crates and the repo-tooling `internals/checks` / `internals/distcheck` uv packages. **There is exactly one exemption**: `no-first-party-patch` on the two `packages/python/tests/integration/hermetic/` suites, waived because patching the first-party `_RustDirSQL` core boundary *is* what makes that tier hermetic -- there is no third-party target the rule could redirect it to, so the rule does not describe that seam. Everywhere else the count stays zero and an exemption is never the escape hatch: barrels get a colocated surface test, logic gets extracted, dead shells get deleted. Full wiring, per-package gate map, and history: `agents/reference/testing-gates.md`.

**Run every gate CI declares before pushing** with `just preflight` (#781), which *derives* the (source, gate) pairs from the CI workflows instead of restating them -- 50 steps across 8 scan roots today (42 gate invocations plus 8 python drift guards; 3 further `packaging` pairs report SKIP), versus the 6 across 3 the four hand-written recipes it replaced covered. Treat that 50 as a snapshot and read the live number off the tool: `just preflight --dry-run | grep -c '^==>'`. It reads every `.github/workflows/*.yml` and folds together each job that calls the reusable workflow, so a lane added in a new per-domain file is a pair it runs. It also encodes the invocations that differ from the naive one (python suites via the package venv, typescript via `npx`, per-gate option support), each of which used to pass locally while failing in CI. `--dry-run` prints the matrix; `--gate <name>` and `--conventions <workflow>` narrow it (both repeatable); `packaging` reports SKIP since it needs a built artifact (`just test-packaging`).

**Commit before you preflight.** The diff-scoped halves of the gates -- `colocated-test`'s co-change check, changed-lines coverage, and `unit mutation` -- read the committed `origin/main..HEAD` range, so on an uncommitted tree they see an empty diff and report success having examined nothing. The whole-tree checks (plain colocation, `unit lint`, `one-function-per-file`) do read the working tree, which is what makes the failure mode deceptive: preflight looks like it ran. Verified by probe -- an uncommitted edit to a source file whose test does not change is silent; the same edit committed fails co-change. #1138 hit exactly this: `0 failing pair(s)` locally, five co-change violations on the first push.
...
CI never runs the e2e suites; instead the `e2e-verify` gate in each package's CI workflow checks, per package, that the branch committed a **receipt** -- one JSON file per branch under the package's `e2e-attestations/` directory (`packages/python/e2e-attestations/<branch>.json`, `packages/ts/e2e-attestations/<branch>.json`), recording the e2e command, its exit code, and the commit it ran against. The branch name is slugified (`/` becomes `-`, lowercased), so `claude/issue-982-xy1u9u` writes `claude-issue-982-xy1u9u.json`; `testing-conventions e2e slug` prints it. The gate reads the `base...HEAD` diff: a branch that leaves the scoped source untouched owes no receipt, and one that changed it passes once its diff adds or updates a receipt -- so **write one receipt per package you changed before you push**: `just e2e-attest-python` / `just e2e-attest-ts` (needs `pip install testing-conventions`). Ordering does not matter: the gate is diff-based, indifferent to commit order, rebases, and squash merges. Because the receipt is keyed by branch, a **branch name must never be reused** -- two PRs on one name write the same receipt path. **Any `packages/rust/src` change (including `cli/`) puts BOTH bindings' scoped source in the diff** (CI-enforced via `[e2e].extra_scope`), so write a receipt in both after core changes. **A binding-crate change puts only its own package in scope** -- `packages/python/src` (PyO3) the python receipt, `packages/ts/napi` the ts one (#933); that per-package scope is why each SDK package carries its own `testing-conventions.toml`. Full mechanics and debugging: `agents/reference/e2e-attestation.md`.
