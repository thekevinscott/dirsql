...
`conventions.yml`'s `internals-checks` call (#494/#503) gates the repo-tooling `internals/checks` package the same way in a **single call** at `internals/checks/src`: `colocated-test`, `unit-lint`, `integration-lint`, `unit-coverage`, `mutation`, and `e2e-verify` (#509). `integration-lint` is folded into this one call (#515) because `thekevinscott/testing-conventions#417` landed on `@v0`: `integration lint` now derives `<package root>/tests/integration/` from the given `path` rather than scanning `path` directly, so pointing it at `internals/checks/src` scans `tests/integration/` and no longer flags the colocated *unit* tests under `src`. Confirmed live against CLI 0.0.77 (`uvx testing-conventions integration lint --language python internals/checks/src` is exit 0; a probe file with an inline patch dropped in `tests/integration/` IS flagged), which retired the separate `internals-checks-integration` call. The pre-existing SDK `python-integration` / `typescript-integration` split predates #417 and folding those is a separate change (#515 non-goal). `tests/integration/` exercises each check's `gate.run()` against real collaborators (real git, real pytest subprocess); `tests/e2e/` spawns the packaged `dirsql-checks` CLI with nothing mocked, gated by `internals/checks/e2e-attestation.json` per the E2E Attestation convention below.
