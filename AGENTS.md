...
`conventions.yml`'s `internals-checks` call (#494/#503) gates the repo-tooling `internals/checks` package the same way: `colocated-test`, `unit-lint`, `unit-coverage`, `mutation`, and `e2e-verify` at `internals/checks/src`, plus a separate `internals-checks-integration` call running `integration-lint` at `internals/checks/tests/integration` (#509). It originally mirrored the `python-integration` / `typescript-integration` split, but those were folded into `python-unit` / `typescript-unit` in #521 once #417 was confirmed live (`integration-lint` now derives its subjects from the package root); `internals-checks-integration` is the last remaining separate integration call, still pending its own fold in #516. That paragraph explains why it needed its own call: a live CI run confirmed the current `@v0` release scans exactly the given `path` for `integration lint` rather than deriving `<package root>/tests/integration/` the way `thekevinscott/testing-conventions#417` / `docs/monorepo.md`'s "The test tiers" describes, so pointing it at `internals/checks/src` wrongly flagged the colocated *unit* tests there. Once #417 lands and is confirmed live (`SCAN_PATH` for `integration lint` resolving to `tests/integration` when `path` is the source dir), fold `internals-checks-integration` back into `internals-checks` and delete the separate call. `tests/integration/` exercises each check's `gate.run()` against real collaborators (real git, real pytest subprocess); `tests/e2e/` spawns the packaged `dirsql-checks` CLI with nothing mocked, gated by `internals/checks/e2e-attestation.json` per the E2E Attestation convention below.
