...
Merge gating is via **pr-monitor** (`.github/workflows/pr-monitor.yml`, the **`CI Gate`** check — [thekevinscott/pr-monitor](https://github.com/thekevinscott/pr-monitor), `@v1` is a rolling tag tracking its `main`). It is an *aggregator*, not a test: it polls the PR head's **workflow runs** and goes green only when every other run has completed with a passing conclusion (`success` / `skipped` / `neutral` / `stale`); `failure`, `cancelled`, `timed_out`, `action_required`, and `startup_failure` all turn it red. Because it watches workflow *runs* rather than check-runs, path-filtered workflows need no enumeration (one that does not run never appears), a still-running workflow holds the gate pending, and a workflow that startup-fails before emitting any check-run still fails the gate. Branch protection requires only `CI Gate`, so adding/renaming/removing jobs or workflows needs no branch-protection or gate-config coordination — don't flag that concern.

**Why not check-run counting (the Mergify era, #830–#947):** a gate built on check-run counts cannot express "everything expected has finished". `#check-failure = 0` alone is vacuously true before CI dispatches and while it runs — a gate that does not gate (#943 measured it green 2m21s before the last check finished). Adding `#check-pending = 0` deadlocks: Mergify counts its own in-progress check, so the count never reaches zero (#832; confirmed deliberately by canary #946 — its check summary showed an empty waiting-checks list with the pending condition unsatisfied). Named per-workflow anchors are the remaining option and path filtering (#834) killed it: an anchor on a workflow that does not run pends forever. Workflow-run polling sidesteps all three, at the cost of one runner for the CI duration.

**The two race guards are `pre-sleep` and `minimum-checks`.** At time T a poller cannot distinguish "all runs finished" from "no runs dispatched yet". `pre-sleep: 30` (wall-clock anchored; setup time is absorbed) covers GitHub's dispatch fan-out, observed at ~2s across nine workflows. `minimum-checks: 0` makes zero runs after the pre-sleep a **pass**: post-#834 a PR can legitimately trigger zero checks, and a check that should have existed but doesn't is the workflow's defect, not the gate's (#943's scope ruling; workflow-config validation belongs to #862). Do not raise `minimum-checks` above 0 — with path filtering there is no floor that is always satisfied, and a floor the diff doesn't meet burns the full timeout as a false red.

**When the gate is red, its log names the culprits** (`Non-passing runs: [...]`). When it **times out** instead, it is a hung job, not load: `timeout: 45` is double the longest run in recorded history (Release Precheck, 22m21s, under multi-PR contention — which is also what killed the original gate's 20-minute timeout), and post-#834 full fan-out completes in ~3 minutes. Open the named in-progress run and look at the stuck job's current *step*: a slow job advances, a hung one sits in one step while its siblings finish in seconds. Cancel it rather than letting it burn its timeout. Learned on #727, where a test awaited an event that a behavior change meant would never arrive: three jobs sat in one step for 35+ minutes and it was read as flake twice before anyone looked at the steps. Note the gate treats `cancelled` as a failure, so a cancelled hung job needs a re-run of that workflow (and the gate) once fixed.

**Gate changes are testable on the PR that makes them.** The gate is an ordinary workflow, so a PR editing `pr-monitor.yml` is gated by its own edited copy — unlike Mergify, which read config from `main` only and forced land-and-canary cycles to test anything.

**Flaky jobs are back to manual handling.** Mergify's CI Insights auto-retry left with Mergify: if a job is flaky, re-run it by hand to unblock, then fix or quarantine it — the gate polls live state, so a re-run that goes green flips the gate without re-triggering anything else. (The gate's own run can be re-run too if it timed out against a since-fixed hang.)
