...
The same workflow also runs `unit lint` -- the **isolation** rule: a unit test must mock every collaborator (it must not import an un-mocked one), so the test exercises only the unit under test. It is wired for **Python and TypeScript**; Rust's `unit lint` is not yet enabled because its inline tests reach for effectful std (`std::fs`, `std::thread`, `std::time`) and the fix is a trait-injected-double refactor of the core's test seams, landing as its own PR (intentional, tracked parity drift -- see issue #233 / epic #231). The fix for a violation is to mock the collaborator (Python: patch it by its dotted path, e.g. `patch("pkg.mod.subprocess.run", ...)`, rather than importing it; TypeScript: `vi.mock("<specifier>")`), or, when a dependency is naturally a callable the unit receives, to inject it (DI) -- never to weaken the test.
