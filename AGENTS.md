...
**The anchor conditions are the part you can break.** `#check-failure = 0` is blind to a workflow that *startup-fails*, because one that dies before emitting a check-run contributes zero check-runs — "nothing failed" is then vacuously true, as it also is before CI has started. **`#check-pending = 0` cannot be used to cover that**: Mergify counts its own in-progress check, so the count never reaches zero and the gate deadlocks (#832). `success_conditions` therefore names one anchor per **unconditionally-triggered** workflow (`Conventions`, `Release Precheck`, `Release Config Check`, `Changelog Check`), matching the caller job id this repo owns rather than inner job names owned by the `@v0` reusable workflows upstream. The anchors are what keep the gate from going green mid-run. Three consequences:
...
- **A path-filtered workflow still running does not hold the gate.** Only the anchored four do, so a PR can go green with e.g. `python-test` still in flight. Narrow in practice — `Release Precheck` is the long pole — but real.
...
- an unmet **anchor** while its workflow is still running → possibly a hung job rather than a slow one. Open the run and look at the job's current *step*: a slow job advances, a hung one sits in one step while its siblings finish in seconds. Cancel it rather than letting it burn its timeout. Learned on #727, where a test awaited an event that a behavior change meant would never arrive: three jobs sat in one step for 35+ minutes and it was read as flake twice before anyone looked at the steps.
