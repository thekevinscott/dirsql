...
- **Drift (Rust only, dirsql#714):** a scan attempts every matched file, skips the ones whose `on-file` hook fails or whose rows the table rejects, commits the rest, and records what it dropped. `DirSQL::scan_failures() -> &[OnFileFailure]` exposes that list, where `OnFileFailure` is a public `{ path, message }` struct. The CLI reports the skips on stderr (capped at ten, then `... and N more`) and exits `23`. Python and TypeScript build successfully in the same situation but have no way to ask which files were skipped, so a caller there sees a silently partial index — tracked in #715, which also adds the `--allow-skipped` opt-out. `DirSqlError::OnFile` and `DirSqlError::OnFileMany` were removed on the Rust side (nothing can produce them); neither binding ever surfaced them, so this narrows no binding's surface.
