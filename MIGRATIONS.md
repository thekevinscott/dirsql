...
### Native-language configs (`.py`/`.js`) and `dirsql interpret` removed; cross-language config serialization retired (#321)

#### Summary

The native-language config path and the `dirsql interpret` subcommand are
**hard-removed** from all three SDKs, with no deprecation window (epic #321:
#323 Python, #324 TypeScript, #325 Rust + docs). The CLI now accepts only
`.dirsql.toml` as a `--config` file; the Rust binary no longer inspects the
config extension or spawns a `dirsql interpret` helper, and `dirsql interpret`
is no longer a subcommand. As a consequence the cross-language config
serialization that existed only to feed the `interpret` handshake (#194) is
retired: Python `DirSQL.__dict__` / `vars(db)` (and `resolve_config`),
TypeScript `DirSQL.toJSON()` / `JSON.stringify(db)` (and `resolveConfig`, plus
the `DirSQLConfig` / `TableConfig` / `ResolvedExtension` types), and Rust
`DirSQL::config()` / `DirSQLConfig` are all removed. The **programmatic SDK**
is unaffected: `DirSQL(...)` with in-process `Table(extract=fn)` closures
remains the supported way to run user-defined extraction in your host
language. TOML configs are unchanged.

Affects: anyone running `dirsql --config <file>.{py,js,mjs,cjs}`, anyone
invoking `dirsql interpret`, and any caller of the serialization snapshot
(`vars(db)` / `db.toJSON()` / `db.config()`).

#### Required changes

| Surface | Before | After |
| ------- | ------ | ----- |
| CLI native-language config | `dirsql --config dirsql.config.py` (or `.js`/`.mjs`/`.cjs`) | Not supported. Convert tables to a `.dirsql.toml` (filesystem-fact columns), or embed the SDK programmatically with `DirSQL(...)` + `Table(extract=fn)` and query it in-process from your own host program. |
| `dirsql interpret <config>` | long-running NDJSON helper subcommand | Removed; the command exits non-zero (unknown subcommand). |
| Python serialized state | `vars(db)` / `db.__dict__` → resolved-config dict | Removed. There is no public serialized-config snapshot; pass `config=`/`root=`/`tables=` into the core and query. |
| TypeScript serialized state | `db.toJSON()` / `JSON.stringify(db)` → `DirSQLConfig` | Removed, along with the `DirSQLConfig` / `TableConfig` / `ResolvedExtension` exported types. |
| Rust serialized state | `db.config()` → `DirSQLConfig` | Removed, along with the `DirSQLConfig` / snapshot `TableConfig` types. |

#### Deprecations removed

_None._ Native configs, `interpret`, and the serialization snapshot were never
deprecated; they are removed in a single release (the feature never shipped a
stable release).

#### Behavior changes without code changes

- `dirsql --config <file>.py` (or `.js`/`.mjs`/`.cjs`) no longer spawns an
  interpreter. The non-TOML file fails to parse as TOML, so the server starts
  in the degraded (HTTP 503) state and `POST /query` returns the parse
  diagnostic instead of serving the config's tables.
- `dirsql interpret …` exits non-zero (clap rejects the unknown subcommand)
  rather than starting an NDJSON helper.

#### Verification

```bash
# 1. `interpret` is no longer a subcommand.
dirsql interpret whatever.py; echo "exit=$?"
# expected: a non-zero exit ("unrecognized subcommand 'interpret'").

# 2. The programmatic SDK still runs in-process extraction (Python).
python - <<'PY'
import asyncio, json, os, tempfile
from dirsql import DirSQL, Table
root = tempfile.mkdtemp()
os.makedirs(os.path.join(root, "a"))
open(os.path.join(root, "a", "meta.json"), "w").write('{"title":"Gamma"}')
...
async def main():
    await db.ready()
    print(await db.query("SELECT title FROM papers"))
asyncio.run(main())
# expected: [{'title': 'Gamma'}]
PY
```

### Python: `DirSQL(...)` no longer raises `TypeError` for `(None, None)` (#260)
...
that originally shipped under #260 are gone with the `interpret` removal above.)
