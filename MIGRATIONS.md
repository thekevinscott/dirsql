...
### `dirsql init` no longer requires or invokes `claude` (#446)

#### Summary

`dirsql init` used to shell out to the `claude` CLI to generate a starter
`.dirsql.toml`, requiring Claude Code installed and signed in, incurring a
paid LLM call, and producing a non-deterministic config. It now writes a
fixed starter config -- byte-for-byte the same single `files` table
zero-config mode already serves -- without inspecting the target
directory's contents at all. This affects every SDK/launcher, since they
all shell out to the same compiled `dirsql` binary for `init`.

#### Required changes

_None._ The command's flags (`--root`, `--output`, `--force`) and exit-code
contract (`0` on success, `1` on failure) are unchanged.
...
- **`init` no longer requires `claude` on `PATH`.** Previously it failed with
  `InitError::ClaudeNotFound` (or `ClaudeFailed` for a non-zero `claude`
  exit) when Claude Code wasn't installed/signed-in; those error arms are
  removed, since there is no longer a subprocess to fail.
- **Generated config content changed.** Previously the LLM inspected the
  directory and chose table boundaries, names, and (sometimes) glob path
  captures; now `init` always writes the same single `files` table
  (`glob = "**/*"`, the seven stat columns) regardless of what's in the
  directory. A config generated by the old `claude`-backed `init` keeps
  working (this only affects newly generated configs); regenerate with
  `--force` to pick up the new fixed shape.
- **Same directory now always produces the same config.** Previously two
  runs over an unchanged directory could differ (different LLM
  outputs/models); now they are byte-identical, and identical regardless of
...
dirsql init
cat .dirsql.toml
# expected (no `claude` invoked, no network access):
# [[table]]
